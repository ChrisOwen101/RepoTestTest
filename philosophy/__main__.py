...
class API(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path != "/" and self.path != "/?q=":
            ...
            if route == "LOOP":
                self.wfile.write(
                    bytes("<strong>" + q + " got stuck in a loop looking for philosophy</strong>", "utf-8")
                )
            elif degrees == False:
                ...
                for each in route:
                    self.wfile.write(
                        bytes(
                            "<li><strong><a href='https://en.wikipedia.org/wiki/"
                            + each[1]
                            + "'>"
                            + each[1]
                            + "</a></strong> leads to</li>",
                            "utf-8",
                        )
                    )
                ...
                self.wfile.write(
                    bytes(
                        "<form action='/' method='post'><button type='submit'>Save output to S3 Bucket</button></form>",
                        "utf-8",
                    )
                )
